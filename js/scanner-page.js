...
function setFrame(state){const el=document.getElementById('scanFrameOverlay'); if(!el) return; el.classList.remove('scan-frame-ok','scan-frame-warn'); if(state==='ok') el.classList.add('scan-frame-ok'); if(state==='warn') el.classList.add('scan-frame-warn');}
...
function parseOcrNumbers(text){
  const found=(String(text||'').match(/\b\d{1,2}\b/g)||[]).map(Number).filter(n=>n>=1&&n<=90);
  const uniq=[]; for(const n of found){ if(!uniq.includes(n)) uniq.push(n); }
...
async function nextImportCode(){
  ...
  let maxOrder=0;
  try{
    const {data}=await client.from('loto_cartons').select('card_order,carton_code,numero').eq('association_id',aid).order('card_order',{ascending:false}).limit(1);
    if(data&&data[0]) maxOrder=Number(data[0].card_order||String(data[0].carton_code||'').match(/(\d{1,4})$/)?.[1]||0);
  }catch(e){}
...
async function saveOcrDraft(grid, quality, rawText){
  ...
  const code=await nextImportCode();
  const numero=codeToNumero(code); const m=code.match(/SDS-(\d{1,2})-(\d{1,4})$/i);
  const row={numero,carton_code:code,association_id:cleanAssociationId(m[1]),card_order:Number(m[2]),numbers_signature:numbersSignatureFromGrid(grid),serie:'IMPORT',lignes:gridToLignes3x9(grid),grille:grid,qr_payload:code,status:'a_enregistrer',origine:'Scan OCR saisie cartons',ocr_quality:quality,ocr_text:String(rawText||'').slice(0,1000),actif:true,updated_at:new Date().toISOString()};
...
async function handleCommissaireCode(value, measured){
  ...
  try{
    stopQrScanner(false); const numero=codeToNumero(value); if(!numero) throw new Error('Code carton illisible : '+value);
    setStatus('QR lu. Contrôle du carton en cours...','muted');
    const payload=await Loto.controlCard(numero); payload.scannedCode=value; payload.readMs=measured; payload.scannedAt=new Date().toISOString();
    try{localStorage.setItem('loto_last_commissaire_scan_result',JSON.stringify(payload));}catch(e){}
    if(payload?.found){await Loto.showPublicCard(payload.result); beep(payload.result?.valid!==false);} else beep(false);
    setTimeout(()=>{location.href='commissaire.html?scan=1&numero='+encodeURIComponent(numero);},250);
  }catch(e){scannerProcessing=false; setStatus('Erreur contrôle : '+(e.message||e),'red'); beep(false);}
}
async function handleCode(value, readMs=null){
  value=String(value||'').trim(); if(!value||scannerProcessing) return;
  const now=performance.now(); if(value===scannerLastValue&&now-scannerLastAt<900) return;
  scannerLastValue=value; scannerLastAt=now; scannerReadCount++; setReadTime(readMs);
...
async function startCamera(){
  const video=document.getElementById('qrScannerVideo'); const deviceId=await getBackCameraId();
  const landscape=scannerUsageMode==='saisie_cartons';
  const videoConstraint=deviceId?{deviceId:{exact:deviceId},width:{ideal:landscape?1280:640},height:{ideal:landscape?720:480}}:{facingMode:{ideal:'environment'},width:{ideal:landscape?1280:640},height:{ideal:landscape?720:480}};
  scannerStream=await navigator.mediaDevices.getUserMedia({video:videoConstraint,audio:false});
  video.setAttribute('playsinline',''); video.setAttribute('webkit-playsinline',''); video.muted=true; video.srcObject=scannerStream; await video.play();
}
async function startCommissaireLoop(){
  const video=document.getElementById('qrScannerVideo'); const canvas=document.createElement('canvas'); const ctx=canvas.getContext('2d',{willReadFrequently:true}); let frame=0;
  setStatus('Scan QR en cours.','muted');
...
async function startSaisieCartonsLoop(){
  ...
  setStatus('Mode saisie cartons : mets le téléphone en paysage et cadre tout le carton. Scan continu.','muted'); setFrame('warn');
  scannerTimer=setInterval(async()=>{
    if(scannerProcessing||!video||video.readyState<2||!window.Tesseract) return;
    scannerProcessing=true;
    try{
      const vw=video.videoWidth||1280, vh=video.videoHeight||720; const sw=Math.min(vw,1100), sh=Math.round(sw*vh/vw); canvas.width=sw; canvas.height=sh; ctx.drawImage(video,0,0,sw,sh);
      const t0=performance.now();
      const result=await Tesseract.recognize(canvas,'eng',{logger:()=>{}});
      const ms=performance.now()-t0; setReadTime(ms);
      const nums=parseOcrNumbers(result?.data?.text||'');
      if(nums.length>=15){
        const grid=buildGridFromNumbers(nums); const quality=Math.round(Math.min(100,Math.max(60,result?.data?.confidence||90)));
        const row=await saveOcrDraft(grid,quality,result?.data?.text||''); setFrame('ok'); beep(true); stopQrScanner(false); setStatus('Carton envoyé dans Administration > Cartons > À enregistrer : '+row.carton_code,'ok');
        setTimeout(()=>{location.href='administration.html#cartons';},700);
      }else{ setFrame('warn'); setStatus('Carton non lu complètement : '+nums.length+'/15 numéros détectés. Continue à cadrer.','muted'); scannerProcessing=false; }
    }catch(e){ setFrame('warn'); setStatus('Analyse impossible : '+(e.message||e),'red'); scannerProcessing=false; }
  },1800);
}
async function startQrScanner(){
  stopQrScanner(false); scannerProcessing=false; scannerLastValue=''; scannerLastAt=0; scannerReadCount=0; setReadTime(NaN); setFrame('');
  const isHttps=location.protocol==='https:'||location.hostname==='localhost'||location.hostname==='127.0.0.1';
  if(!isHttps){setStatus('Caméra bloquée : ouvrir en HTTPS.','red'); return;}
  if(!navigator.mediaDevices?.getUserMedia){setStatus('Caméra non disponible.','red'); return;}
...
function stopQrScanner(showMessage=true){
  if(scannerTimer) clearInterval(scannerTimer); scannerTimer=null; if(scannerStream){scannerStream.getTracks().forEach(t=>t.stop()); scannerStream=null;}
  const video=document.getElementById('qrScannerVideo'); if(video){video.pause?.(); video.srcObject=null;}
  if(showMessage) setStatus('Caméra arrêtée.','muted'); scannerProcessing=false; setFrame('');
}
function initPage(){
  const title=document.getElementById('scanPageTitle'); const help=document.getElementById('scanPageHelp'); const back=document.getElementById('scanBackLink');
  if(scannerUsageMode==='saisie_cartons'){ if(title) title.textContent='Scanner saisie cartons'; if(help) help.textContent='Téléphone en paysage. Cadre tout le carton : la détection est continue, puis le carton part dans Administration > Cartons > À enregistrer.'; if(back) back.href='administration.html#cartons'; document.body.classList.add('scan-saisie-mode'); }
  else { if(title) title.textContent='Scanner commissaire'; if(help) help.textContent='Scanne le QR du carton. Après lecture, retour automatique vers la page commissaire.'; if(back) back.href='commissaire.html'; }
}
