...
function renderLot(s){const p=Loto.currentPartie();const prize=Loto.currentPrize();if(!p||!prize){currentLot.innerHTML='<b>Lot en cours :</b> aucun programme saisi';return;}currentLot.innerHTML=`<b>${p.name||'Partie'}</b> · ${prize.type||''} · <b>${prize.label||'Lot non renseigné'}</b>`;}
