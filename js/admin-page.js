...
async function confirmGeneratedCards(){
  ...
  try{
    ...
    const fullRows=generatedCards.map(c=>({numero:c.numero,carton_code:c.code,association_id:c.associationId,card_order:c.cardOrder,numbers_signature:c.numbersSignature,serie:c.serie,lignes:c.lignes,grille:c.grid,sheet_code:c.sheetCode,sheet_position:c.sheetPosition,support_type:c.supportType||'C',qr_payload:c.qrPayload,status:c.status,origine:c.origin,actif:true,updated_at:new Date().toISOString()}));
...
function buildGeneratedCards({startOrder,count,serie,associationId='01',layout='individual4'}){
  ...
  for(let i=0;i<count;i++){
    ...
    const code=cartonCodeFromOrder(order,aid);
    const supportType=layout==='sheet4'?'P4':layout==='sheet8'?'P8':(layout==='sheet6plus2'&&inPlanche?'P6':'C');
    generatedCards.push({numero:cardInternalNumero(aid,order),cardOrder:order,code,associationId:aid,serie,grid,lignes:gridToLignes(grid),numbersSignature:sig,sheetNumber:inPlanche?pageIndex+1:null,sheetCode:sc,sheetPosition:inPlanche?pos:null,supportType,qrPayload:inPlanche?sc:code,status:'disponible',origin:'Loto by SdS'});
  }
  renderGeneratedCards();
...
function rowToGeneratedCard(c){
  return {numero:c.numero,cardOrder:c.card_order,code:c.carton_code,associationId:c.association_id,serie:c.serie,grid:normalizeGrid3x9(c.grille,c.lignes),lignes:c.lignes,numbersSignature:c.numbers_signature,sheetCode:c.sheet_code,sheetPosition:c.sheet_position,supportType:c.support_type||'C',qrPayload:c.qr_payload||c.carton_code,status:c.status,origin:c.origine};
}
